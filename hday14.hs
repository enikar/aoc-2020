...
part1 :: [Instr] -> Int
part1 instrs = sum memory
  where
    -- we fill the memory (an IntMap), in walking along instructions.
    (_mask, memory) = foldl' f ("", M.empty) instrs

    f (_, mem) (IM mask) = (mask, mem)
    f (mask, mem) (IS (MSet addr val)) = (mask, M.insert addr val' mem)
      where
        val' = (val .&. mAnd) .|. mOr -- apply the mask
        (mAnd, mOr) = buildMask mask

part2 :: [Instr] -> Int
part2 instrs = sum memory
  where
    (_mask, memory) = foldl' f ("", M.empty) instrs

    f (_, mem) (IM mask) = (mask, mem)
    f (mask, mem) (IS (MSet addr val)) = (mask, mem')
      where
        addresses = genAddr (applyMask mask (intToBinStr addr))

        mem' = foldl' g mem addresses

        g acc x = M.insert x val acc

-- applies the mask for part2. Unlike for part1 we need use string
-- to manage floatting bits
...
-- other utilities
-- build the two parts of the mask for part1
...
buildMask str = foldl' f (0, 0) str
  where
    f (mAnd, mOr) c
      ...
      |c == '0' = (2*mAnd, 2*mOr)
      |otherwise = error ("Error: buildMask: Invalid character: " <> show c)
